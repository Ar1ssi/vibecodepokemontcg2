...
- A click on interactive content inside the card carousel is silently ignored (no console error, no chat/battle-log line, no action) → `attachSwipe`'s `onPointerDown` calls `stage.setPointerCapture()`, and the browser then dispatches the follow-up `click` to the capturing stage, not the hit-test target, so a bubbling `click` listener on a stage *descendant* (the design-013 inspector chrome) never fires → don't delegate clicks from carousel-slide content that isn't in `isSwipeBlockedTarget`; add the target (e.g. `.ptcg-chrome`) there so pointer capture is skipped. A programmatic `el.click()` bypasses pointerdown and will NOT reproduce this — verify carousel clicks with real input.
