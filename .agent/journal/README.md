...
# Grep it or `tail -n 20` it — never load whole files into context. Monthly files (<YYYY-MM>.md), created on first write.
# maintain.md rolls months older than 2 into ARCHIVE.md (one line per session, live flags kept).

Entry format (≤4 lines per session, ≤200 chars per line — detail goes in designs/scratch):
