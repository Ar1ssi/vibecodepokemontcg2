...
Active: none (slice 2 completed and verified)
Next: Slice 3 — commands.mjs vocabulary + applyCommand for moves, draws, attachments; server GameRoom skeleton behind flag.
Blocked: none
...
- Test baseline: `pnpm test` → 828 pass / 0 fail (46 `.mjs` files, plain `node --test`, no jsdom).
