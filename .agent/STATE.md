...
Session: 249
Focus: Mega Greninja ex "Ninja Spinner" — optional return-Energy-for-damage prompt.
Active: S249 uncommitted on main (damage-parser, attack-damage-context, reduce, attack-effects-execution.test).
Next: push S243-S249 if user approves; optional Live parity extras (hand chevron, edge wisps).
Blocked: nothing.
...
- ONE pre-existing failing test: `card-inspector-model.test.mjs` "retreat greys only when the cost is unpaid" — fails at HEAD too (verified via stash), unrelated to design 023. Suite is otherwise 2815/2816.
