...
Active: none (Slice 4 complete)
Next: Slice 5 — transcription of turn loop, attack resolution, and step executors
Blocked: none

## Watch-outs (≤5 — things the next session must know; prune ruthlessly)
- `pnpm lint` fails on Windows CRLF line endings across existing codebase; lint changed files with `npx eslint <files>`.
- Shared modules in `shared/engine/` must import relatively and stay DOM-free (Invariants 6, 8 verified by invariants.test.mjs).
- Test baseline: `pnpm test` → 861 pass / 0 fail (51 `.mjs` files, plain `node --test`, no jsdom).
