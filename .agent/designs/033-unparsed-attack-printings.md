...
- Timed effects are `card.attackMarkers` (D109), read only while the card is Active.
...
2. Encore / "can't use that attack" (Unown Amnesia shares the handler).
   ...
   legality gate. Pick B (D109: one marker list, evolve/retreat/KO expiry for free).
