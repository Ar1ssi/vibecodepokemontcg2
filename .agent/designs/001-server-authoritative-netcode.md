...
| 7 | Server crash / restart | In-memory game lost by design (decision 9). Both clients get `gameEnded` `server_restart` and return to the room screen cleanly — no half-dead board, no silent hang | [x] covered: server/game/__tests__/slice8-edge-cases.test.mjs |
| 8 | `view` arrives out of order / duplicated | `stateVersion <= lastRendered` ignored; snapshot semantics make this safe | [x] covered: client/src/setup/netcode/__tests__/apply-view.test.mjs |
...
| 14 | Third player joins a full room | Existing `roomReject` preserved | [x] covered: server/game/__tests__/slice8-edge-cases.test.mjs |
...
| 16 | TCGdex `fetch` for card data fails/times out | Card DB preloaded and cached server-side at game start; a failed lookup degrades to decklist data, never blocks a command | [x] covered: server/game/__tests__/slice8-edge-cases.test.mjs |
| 17 | Legacy client connects to new server (or vice versa) | Protocol version in `joinGame`; mismatch → explicit "reload the page" message, not a silent desync | [x] covered: server/game/__tests__/slice8-edge-cases.test.mjs |
| 18 | Solo (single-player) mode | Runs the same engine in-process client-side; must not regress | [x] covered: server/game/__tests__/slice8-edge-cases.test.mjs |
...
- **Slice 7**: Implemented `view-diff.mjs` for pure structural diffing between views and `apply-view.js` for authoritative DOM reconciliation with monotonic `stateVersion` guard (Edge Case 8), identity-stable `instanceId` mapping, and interactive `PendingChoice` modal renderer. Added `cmd-emitter.js` with monotonic `clientSeq` tracking, in-flight affordances (`.cmd-pending`), and `resolveChoice`/`requestView` socket actions. Integrated Phase 2 dual-run translation in `process-action.js` and `dual-run-bridge.js`. Added `socket.on('resolveChoice')` in `server/server.js` and verified end-to-end multi-client convergence via `dual-run-sync.test.mjs`.
- **Slice 8**: Flipped default server configuration to authoritative (`SERVER_AUTHORITATIVE=true` by default unless explicitly disabled). Server negotiates `PROTOCOL_VERSION = '2.0.0'` on `joinGame` and validates client protocol versions on commands (Edge Case 17). Server crash / restart emits `gameEnded` with `reason: 'server_restart'`, allowing client to cleanly reset and return to the lobby (Edge Case 7). Enforced maximum 2 players per room, rejecting 3rd players with `roomReject` while admitting spectators (Edge Case 14). Preserved command execution without crashing on missing TCGdex data (Edge Case 16). Preserved in-process solo mode execution (Edge Case 18). Executed Phase 4 deletion pass: eliminated legacy client-side reconciliation files (`catch-up-actions.js`, `resync-actions.js`, `sync-replay.mjs`, `request-board-snapshot.js`, `apply-board-snapshot.js`, `board-hash.js`, `rules-local-effects.mjs`), while retaining pure zone hashing (`board-snapshot.mjs`, `zone-hash.mjs`) in `shared/engine/zones/`. All 20 edge cases across the netcode design are now covered and verified.
