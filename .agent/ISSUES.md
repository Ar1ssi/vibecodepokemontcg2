...
- I32 2026-09-10 P1 [rules/netcode] Retreat on a FULL bench desyncs the peer — HALF FIXED S87, mirror side still open. Root cause of the local half: `retreat()` did two untargeted moves (active→bench, then bench→active), and with 5 on the bench the first one asks for a 6th, so move-card.js's bench-limit gate (move-card.js:279-292, which only applies when no `targetCard` was named) rejected it and returned ok:false — leaving the retreating Pokémon in the active zone beside the newly promoted one, wrecking every later bench play. Fixed S87 by making it ONE targeted move, so the gate is skipped and autoMoveActiveBenchCard's case 3 (auto-move-active-bench-card.js:57-75) performs the swap; the bench never exceeds its limit at any point. My original "high bench index" framing was wrong — index correlated with bench fullness, it was not the variable (refs: I30, design 004, S86, S87).
