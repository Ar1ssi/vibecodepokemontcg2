...
- I33 2026-09-10 P1 [rules/netcode] `pass` reports success but never ends the turn, so a game wedges: with I32 fixed, games reach turn 10-14 and then the SAME side (always 'b', the joiner) passes 60 times in one turn, every act reporting ok. Both boards agree at that point (aSelf == bOpp in the dumps), so this is not a divergence. Repro `--scorer=coverage --seed=7|123` (2/2 each); dumps `out/playtest/7-{0,1}-*.json`, `123-{0,1}-*.json` (refs: I29, design 004, S87).
    Two things to separate, and the first may be all of it: `__ptcg.act()` computes `{ ok: ok !== false }`, but `pass()` returns undefined on BOTH its success path and its blocked path (the `⛔` appendMessage + bare `return`), so a pass the rules gate rejected is indistinguishable from one that worked — fix `act()` to report the block, then the dumps will say why the gate refused. Second, and only once the above reports honestly: whether it is a real gate disagreement on the joiner, or the harness's own turn-detection reading `a.page`'s turnState to decide whose turn it is — which is exactly the shape of I29, closed S82 as a harness race rather than an engine bug. Do not assume an engine bug before `act()` tells the truth.
