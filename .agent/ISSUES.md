...
- I37 2026-09-11 P2 [netcode] Move-then-shuffle sequences other than deck searches (hand→deck then shuffle: Judge/Iono-style effects, stadium hand-to-deck) were not audited for design 007's emit-order bug (shuffle relayed before an un-awaited move). Mirrors now keep every card and log `shuffleZone.indices_mismatch` when it happens — grep that event in future sync logs before assuming a new cause (refs: design 007, S91).
- I36 2026-09-11 P3 [netcode] After a KO, the peer's mirror already had the owner's promoted Pokémon in Active when the owner's bench→active move arrived, so it aborted `hint_mismatch` (Fezandipiti ex, then Eelektrik). Final boards matched; what moved it on the mirror is unknown. Log `ptcg-sync-log_combined_test_1789066196357.json`, MamoswineEnjoyer seq 98-99 and 184-185 (refs: S91).
