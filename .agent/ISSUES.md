...
- I37 2026-09-11 P2 [netcode] AUDITED S95, NARROWER root cause than filed: `shuffleHandThenDraw`/`countShuffleDrawPlus`/`ionoShuffle`/`discardHandThenDraw` (trainer-execution.js:717-758 — Judge/Iono/N/Professor's-Research-style effects) never call `_shuffleZone` at all, so design 007's specific move-vs-shuffle race can't occur here — no shuffle relay exists to race against. A DIFFERENT, real bug instead: each of these loops fires `moveCardBundle` in a tight `for`/`while` without `await`ing between iterations (same shape `movePicksInOrder`/design 007 fixed for card-picker multi-search) — `moveCardBundle`'s synchronous prefix (move-card-bundle.js:199-210, `buildMoveCardHints`) reads the zone array before the PRIOR un-awaited call's own `moveCard` has actually spliced it, so every iteration after the first sends a stale/wrong `cardHints.moving` to the peer. Separately (correctness, not sync): none of these four ever randomize the deck — cards are pushed to index 0 one at a time then drawn back from index 0, so "shuffle your hand into the deck" deals the same cards back in reverse-of-reverse order with no real randomization, unlike the authoritative engine's `executor.mjs:365-382` (which does call `activeRng.shuffle(deck)`). Not fixed this session — needs the same `movePicksInOrder`-style sequential-await treatment as design 007, plus deciding whether to add a real shuffle call. Repro: play Judge/Iono/N in a legacy 2P game while both hands are non-empty; check sync log for `cardHints.moving` matching the wrong card index (refs: design 007, S91, S95).
- I36 2026-09-11 P3 [netcode] INVESTIGATED S95, LIKELY BENIGN: KO auto-promotion (chat-buttons.js:1184-1207, `moveCard(oppPlayer, user, 'bench', 'active', 0)`) is called directly — not via `moveCardBundle` — from inside `attack()`, which both clients replay identically off the same seeded RNG bundle (S87's `isMirrorReplayCall` fix). It carries no `cardHints` and is never relayed: each client derives the same promotion independently and deterministically, so the peer's bench/active should already match right after a KO without needing a wire message. The reported `hint_mismatch` abort is consistent with a genuinely separate, relayed `moveCardBundle` (e.g. a manual retreat/switch) whose hints were captured against a pre-promotion bench index landing around the same time — the abort is the safety net correctly refusing a stale relay, which is why "final boards matched" (the promotion already happened correctly on both sides via replay; the aborted relay was redundant/stale, not lost). Not fixed — P3, no live symptom, no repro currently available (source log no longer on disk). If it recurs, check whether the relayed move's timestamp brackets the KO/promotion on the sender's side (refs: S91, S95).
