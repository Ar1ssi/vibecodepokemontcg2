...
const info = await page.evaluate(async ([card, fire, trainer]) => {
  ...
  window.__cf = await import('/src/setup/netcode/mat-fx/card-flight.mjs');
  ...
  window.__discard = () => {
    const { cardShapedRect } = window.__cf; const t = selfRect.width * 0.24; const from = cardShapedRect({ left: selfRect.left + 2, top: selfRect.top + selfRect.height * 0.75, width: t, height: t });
    const from2 = cardShapedRect({ left: selfRect.left + 2 + t * 1.15, top: selfRect.top + selfRect.height * 0.75, width: t, height: t });
    playDiscardFlights([{ src: fire, rect: from, turn: 0 }, { src: fire, rect: from2, turn: 0 }], selfPile);
  };
  return { pile, selfPile, turn, oppRect };
}, [CARD, FIRE, TRAINER]);
console.log(JSON.stringify(info));
await page.waitForTimeout(600);
await page.evaluate(() => document.querySelector('img[style*="2400"]')?.remove());
await page.waitForTimeout(300);
await page.evaluate(() => window.__discard());
await page.waitForTimeout(1500);
const v = page.video();
await ctx.close();
renameSync(await v.path(), 'out/discard-token.webm');
await browser.close();
console.log('ok');
