...
// Live TCGdex smoke test — opt-in via `pnpm test:live` (LIVE_TESTS=1) so network flakiness never
// reds the default suite (I182). Verifies the real PFL Piloswine incident: collector number "24"
// alone must not resolve to Skyridge (ecard3-24).
const { ensureCardData, resolveCardId } = await import('../rules-state.mjs');

const skipLive = !globalThis.fetch || process.env.LIVE_TESTS !== '1';

test('live: PBL Popplio #18 resolves to Pitch Black (me05-018)', { skip: skipLive }, async () => {
...
test('live: PFL Piloswine #24 resolves to Phantasmal Flames (me02-024)', { skip: skipLive }, async () => {
...
test('live: name search with number only still ambiguous without set code', { skip: skipLive }, async () => {
