...
#### 14. Overwritten Stadium Cards Erased Without Discarding — [RESOLVED]
* **Location**: [`shared/engine/effects/trainer.mjs:22-87, 160-168, 183-193`](file:///c:/Users/SMG26/.gemini/antigravity/scratch/vibecodepokemontcg2/shared/engine/effects/trainer.mjs#L22-L87), [`shared/engine/reduce.mjs:612-620, 822-830`](file:///c:/Users/SMG26/.gemini/antigravity/scratch/vibecodepokemontcg2/shared/engine/reduce.mjs#L612-L620), [`shared/engine/state.mjs:85-93, 136-146`](file:///c:/Users/SMG26/.gemini/antigravity/scratch/vibecodepokemontcg2/shared/engine/state.mjs#L85-L93)
* **Root Cause**: When a new Stadium is played, `draft.stadium = played` replaces the previous stadium. The existing stadium card was never moved to its owner's `discard` zone, silently erasing displaced stadium cards from the game. Furthermore, stadium cards had no explicit `ownerId` tracking across players, cards in stadium reported `playerId: null` in `findCard`, and players could illegally play duplicate Stadium cards with the same name.
* **Fix**:
  1. Implemented and exported `discardCurrentStadium(draft, events, initiatorPlayerId)` in [`shared/engine/effects/trainer.mjs`](file:///c:/Users/SMG26/.gemini/antigravity/scratch/vibecodepokemontcg2/shared/engine/effects/trainer.mjs) which reliably resolves the owner of the active stadium (via `ownerId`, `playerId`, or `deckList` card identity fallback) and moves it into the owner's `discard` zone with a `cardMoved` event (`from: 'stadium', to: 'discard'`).
  2. Updated `executeTrainer` in [`shared/engine/effects/trainer.mjs`](file:///c:/Users/SMG26/.gemini/antigravity/scratch/vibecodepokemontcg2/shared/engine/effects/trainer.mjs) across both initial play and choice resume cleanup to invoke `discardCurrentStadium`, stamp `ownerId: playerId` on the new stadium, and reset `stadiumUsedThisTurn` for players upon a new stadium entering play.
  3. Updated `moveCard` in [`shared/engine/reduce.mjs`](file:///c:/Users/SMG26/.gemini/antigravity/scratch/vibecodepokemontcg2/shared/engine/reduce.mjs) when moving a card `to: 'stadium'` to displace any existing stadium to its owner's discard zone and tag `ownerId`.
